...
def _get_paths_from_exposures(path):
    '''get exp path list from image folder'''
    assert os.path.isdir(path), '{:s} is not a valid directory'.format(path)
    exposures = []
    for dirpath, _, fnames in sorted(os.walk(path)):
        for fname in sorted(fnames):
            if is_image_file(fname):
                exp_path = os.path.join(dirpath, fname)
                exposures.append(exp_path)
    assert exposures, '{:s} has no valid file'.format(path)
    return exposures
...
def get_exp_paths(dataroot):
    '''get image path list
    support lmdb or image files'''
    paths = None
    if dataroot is not None:
        paths = sorted(_get_paths_from_exposures(dataroot))
    else:
        raise NotImplementedError('dataroot_exposures is not right.')
    return paths

def get_paths(dataroot, fname):
    paths = sorted(glob.glob(osp.join(dataroot, fname)))
    return paths
